...
import { 
  LayoutDashboard, Server, FileCode2, Scale, Crown, 
  TrendingUp, CreditCard, Activity, LogOut, CheckCircle2, 
  AlertCircle, Building2, Zap, ArrowRight, Wallet, Lock, ArrowRightLeft, Combine
} from 'lucide-react';
...
export default function App() {
  ...
  const authenticatePiUser = async (isAuto = false) => {
    ...
    try {
      ...
      await withTimeout(window.Pi.init({ version: "2.0", sandbox: true }), 2000, "Pi init timeout");
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 flex">
      ...
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col fixed inset-y-0 z-20">
        <div className="p-6 flex flex-col items-center border-b border-slate-800">
          ...
          <h1 className="text-xl font-bold text-white tracking-tight">PiRC Alpha Hub</h1>
          <p className="text-xs text-emerald-400 mt-1 font-medium bg-emerald-400/10 px-2 py-0.5 rounded-full border border-emerald-400/20">Unified Gateway</p>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          {isAuthenticating ? (
            ...
            <div className="space-y-3">
              ...
              >
                <img src="https://storage.googleapis.com/e-object-409003.firebasestorage.app/logo-black-tiny.png" className="h-4 invert" alt="Pi" />
                Sign in with Pi
              </button>
            </div>
          )}
        ...
        <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-10">
          <div className="h-16 flex items-center px-8 overflow-hidden bg-slate-950 text-white">
            <div className="flex items-center space-x-2 mr-8 shrink-0">
               ...
               <span className="text-xs font-bold tracking-wider text-slate-400 uppercase">Live Exchange</span>
            </div>
            <div className="flex space-x-12 animate-[marquee_20s_linear_infinite] whitespace-nowrap">
               {exchangeTickers.map((t, i) => (
                 <div key={i} className="flex items-center space-x-2">
                   ...
                   <span className={cn("text-xs font-semibold", t.up ? 'text-emerald-400' : 'text-rose-400')}>
                     {t.change}
                   </span>
                 </div>
               ))}
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-8">
          ...
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'dashboard' && (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-3xl font-bold mb-2">Systems Overview</h2>
                    <p className="text-slate-500 dark:text-slate-400">Gateway to all ecosystem projects and market data.</p>
                  </div>
                  {user && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                      <h3 className="font-bold text-lg flex items-center gap-2"><Activity className="w-5 h-5 text-emerald-500"/> Core Token Valuation</h3>
                    ...
                    <div className="h-72">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={mockPriceData}>
                          ...
                          <YAxis domain={['dataMin - 1', 'dataMax + 1']} tick={{ fill: '#64748b' }} axisLine={false} tickLine={false} />
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                      <h3 className="font-bold text-lg flex items-center gap-2"><Wallet className="w-5 h-5 text-indigo-500"/> Synchronized Purchasing Power</h3>
                      <span className="text-xs bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 px-2 py-1 rounded-full font-bold">Network Aggregate</span>
                    </div>
                    <div className="h-72">
                       <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={scMetrics}>
                          ...
                          <RechartsTooltip cursor={{fill: 'transparent'}} contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '8px', color: '#fff' }} />
            ...
            {activeTab === 'trading' && (
              ...
              >
                 <div>
                    <h2 className="text-3xl font-bold mb-2 flex items-center gap-3"><TrendingUp className="text-emerald-500" /> Smart Contract Trading</h2>
                    <p className="text-slate-500 dark:text-slate-400">Manage assets, execute smart contracts, and monitor live stock exchange flows.</p>
                 </div>
                 ...
                 <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                   ...
                   <div className="lg:col-span-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col">
                     <h3 className="font-bold mb-6 text-lg border-b border-slate-100 dark:border-slate-800 pb-4">Execute Smart Contract Swap</h3>
                     
                     <div className="space-y-4 flex-1">
                       <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                         <div className="text-xs text-slate-500 dark:text-slate-400 mb-1 font-medium">You Pay</div>
                         <div className="flex justify-between items-center">
                           <input type="text" className="bg-transparent text-2xl font-semibold outline-none w-1/2" placeholder="0.0" defaultValue="100.0" />
                           <span className="bg-white dark:bg-slate-950 px-3 py-1 rounded-full text-sm font-bold shadow-sm border border-slate-200 dark:border-slate-700">PI</span>
                         </div>
                       </div>
                       ...
                       <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                         <div className="text-xs text-slate-500 dark:text-slate-400 mb-1 font-medium">You Receive (Est.)</div>
                         <div className="flex justify-between items-center">
                           <input type="text" className="bg-transparent text-2xl font-semibold outline-none w-1/2 text-slate-500" placeholder="0.0" value="3820.0" readOnly />
                           <span className="bg-white dark:bg-slate-950 px-3 py-1 rounded-full text-sm font-bold shadow-sm border border-slate-200 dark:border-slate-700">USD</span>
                         </div>
                       </div>
                     </div>
                     ...
                     <button className={cn(
                       ...
                       user ? "bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/25" : "bg-slate-400 dark:bg-slate-700 cursor-not-allowed opacity-50"
                     )}>
                   ...
                   <div className="lg:col-span-2 bg-slate-950 rounded-2xl border border-slate-800 p-6 flex flex-col shadow-xl">
                      <div className="flex justify-between items-center mb-6 border-b border-slate-800 pb-4">
                        <h3 className="font-bold text-lg text-slate-100 flex items-center gap-2"><Server className="w-5 h-5 text-emerald-400"/> Live Orderbook & Routing</h3>
                      </div>
                      <div className="flex-1 font-mono text-xs text-slate-400 space-y-2 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                        <div className="text-sky-400">[SYS] Establishing sovereign routing nodes... OK.</div>
                        <div className="text-emerald-400">[SC] Block 49210 validated.</div>
                        <div className="text-emerald-400 text-opacity-80">[TRADE] Executed: 400.0 PI → 15280.0 USD @ 38.20</div>
                        <div className="text-slate-500">[MEMPOOL] Scanning for standard transactions...</div>
                        <div className="text-amber-400">[WARN] Price slippage detected on PIRC/ETH pair. Routing alternative paths.</div>
                        <div className="text-emerald-400">[SC] Smart Contract execution successful. Hash: 0x8a1b...9f2c</div>
                        <div className="text-sky-400">[SYS] Syncing purchasing power parameters globally...</div>
                        {Array.from({length: 10}).map((_, i) => (
                           <div key={i} className="text-slate-600">[DATA] Telemetry heartbeat {Date.now() - i*1000}</div>
                        ))}
                      </div>
                   </div>
                 </div>
            ...
            {activeTab === 'technical' && (
              ...
              >
                 <div>
                    <h2 className="text-3xl font-bold mb-2 flex items-center gap-3"><FileCode2 className="text-indigo-500" /> Technical Aspects Archive</h2>
                    <p className="text-slate-500 dark:text-slate-400">Access documentation, architectural blueprints, and protocol specifications.</p>
                 </div>
                 <div className="grid md:grid-cols-2 gap-6">
                    <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800">
                      ...
                      <h3 className="text-xl font-bold mb-2">PiRC Protocol Standards</h3>
                      <p className="text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">The definition for uniform integration of modules and applications across the ecosystem. Guaranteeing scalability and decentralized integrity.</p>
                      <button className="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500">Read Protocol Specs &rarr;</button>
                    </div>
                    <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800">
                      <Combine className="w-8 h-8 text-amber-500 mb-4" />
                      <h3 className="text-xl font-bold mb-2">Warehouse Features</h3>
                      <p className="text-slate-600 dark:text-slate-400 mb-6 leading-relaxed">View all centralized storage logic, data hashing methodologies, and asynchronous sync routines governing data replication.</p>
                      <button className="text-sm font-semibold text-amber-600 dark:text-amber-400 hover:text-amber-500">View Infrastructure &rarr;</button>
                    </div>
                 </div>
              </motion.div>
            )}
            ...
            {activeTab === 'sovereign' && (
              ...
              >
                 <div className="rounded-2xl bg-gradient-to-br from-amber-500/10 to-orange-600/10 border border-amber-200 dark:border-amber-900/40 p-10 flex flex-col items-center text-center">
                    ...
                    <h2 className="text-4xl font-bold mb-4 text-amber-900 dark:text-amber-100">Sovereign System</h2>
                    <p className="text-amber-800/70 dark:text-amber-200/70 max-w-2xl text-lg leading-relaxed mb-8">
                      The Sovereign module enables absolute autonomy over digital identities and assets. It guarantees self-sovereignty enforced by rigid smart contracts and verifiable mathematical proofs, entirely outside centralized control structures.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl text-left">
                       <div className="bg-white/50 dark:bg-slate-950/50 p-6 rounded-xl border border-amber-200 dark:border-amber-800/30">
                          <h4 className="font-bold text-amber-900 dark:text-amber-100 mb-2">Identity Autonomy</h4>
                          <p className="text-amber-800/60 dark:text-amber-200/60 text-sm">You possess the private keys to your digital life without intermediary proxies.</p>
                       </div>
                       <div className="bg-white/50 dark:bg-slate-950/50 p-6 rounded-xl border border-amber-200 dark:border-amber-800/30">
                          <h4 className="font-bold text-amber-900 dark:text-amber-100 mb-2">Uncensorable Vaults</h4>
                          <p className="text-amber-800/60 dark:text-amber-200/60 text-sm">Value is held in multi-sig sovereign vaults governed directly by the community.</p>
                       </div>
                       <div className="bg-white/50 dark:bg-slate-950/50 p-6 rounded-xl border border-amber-200 dark:border-amber-800/30">
                          <h4 className="font-bold text-amber-900 dark:text-amber-100 mb-2">Absolute Voting Rights</h4>
                          <p className="text-amber-800/60 dark:text-amber-200/60 text-sm">Weighted sovereign liquid democracy ensures your voice correlates dynamically to stake.</p>
                       </div>
                    </div>
                 </div>
              </motion.div>
                 ...
                 <div className="rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 p-10 flex flex-col md:flex-row items-center gap-12 text-white overflow-hidden relative">
                    ...
                    <div className="flex-1 z-10">
                      <div className="inline-flex items-center space-x-2 bg-indigo-500/20 text-indigo-300 px-3 py-1 rounded-full text-xs font-bold mb-6 uppercase tracking-wider">
                        <Scale className="w-3 h-3" /> <span>Automated Arbitrage</span>
                      </div>
                      <h2 className="text-4xl md:text-5xl font-bold mb-6 tracking-tight">Divine Justice Framework</h2>
                      <p className="text-slate-400 text-lg leading-relaxed mb-6">
                        An immutable, code-driven arbitration protocol. Malicious actors, rug-pulls, and exploiters are automatically penalized through slashed stakes via deterministically enforced smart contracts. Justice is blind, automated, and absolute.
                      </p>
                      <button className="bg-white text-slate-900 px-6 py-3 rounded-lg font-bold hover:bg-slate-200 transition-colors">Review Audit Logs</button>
                    </div>
